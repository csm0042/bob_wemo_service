...
class WemoAPI(object):
    ...
    def read_status(self, name, address, status, last_seen):
        ...
        self.logger.debug('Querrying device [%s] at [%s], original status '
                          '[%s / %s]',
                          name,
                          address,
                          status,
        ...
        self.result = next(
            (index for index, wemodev in enumerate(self._wemo_known)
        ...
        if self.wemo_device is not None:
            ...
            self.logger.debug('Wemo device [%s] found with status [%s]',
                              name, self.status)
            ...
            self.status = 'offline'
            self.logger.debug('Wemo device [%s] discovery failed.  '
                              'Status set to [%s]', name, self.status)
            status = copy.copy(self.status)
    ...
    def turn_on(self, name, address, last_seen):
        ...
        self.logger.debug('Setting device [%s] at [%s], state to "on"',
                          name, address)
        # Check if device is already in the list of known wemo devices
        self.result = next(
            (index for index, wemodev in enumerate(self._wemo_known)
        ...
        if self.result is None:
            ...
            self.logger.debug('Device already in wemo list as [%s]',
                              self._wemo_known[self.result])
            self.wemo_device = self._wemo_known[self.result]
        # Perform command, followed by status query
        if self.wemo_device is not None:
            self.wemo_device.on()
            self.status = 'on'
            self.logger.debug('"on" command sent to wemo device [%s]',
            ...
            self.status = 'offline'
            self.logger.debug('Wemo device [%s] discovery failed.  Status set to [%s]',
                              name, self.status)
        return self.status, last_seen
    ...
    def turn_off(self, name, address, last_seen):
        ...
        self.logger.debug('Setting device [%s] at [%s], state to "off"',
                          name,
                          address)
        # Check if device is already in the list of known wemo devices
        self.result = next(
        ...
        if self.wemo_device is not None:
            ...
            self.status = 'off'
            self.logger.debug('"off" command sent to wemo device [%s]',
                              self.wemo_device.name)
            # Re-define device record based on response from status query
            last_seen = str(datetime.datetime.now())
            # If device was not previously in wemo list, add it for next time
